<?php


?>
<head>
    <link rel="stylesheet" type="text/css" href='css/whoweare.css'>
</head>

<section>
    <div class="banner_whoweare">
        <img src="resources/imgs/banner.jpg">
        <h1 style="color: teal; text-align: center">QUI INVESTIT DANS CETTE IDÉE?</h1>
    </div>
</section>

<section class="mosaico">
    <div class="container_WWA">
        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/daniel.jpg');" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico">
                <h2>Daniel PINHEIRO MELO</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->

        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/joao.jpg');order:2;" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico" style="order:1;">
                <h2>João Paulo PINHEIRO MELO</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->

        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/matheus.jpg');" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico">
                <h2>Matheus GUIMARÃES DE MOURA</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->

        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/jones.jpg');order:2;" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico" style="order:1;">
                <h2>João Lucas SANTOS PENHA DE OLIVEIRA</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->

        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/joel.jpg');" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico">
                <h2>Joel Kalil DA SILVA PONTES</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->

        <div class="flex-mosaico">
            <div style="background-image:url('resources/imgs/yishan.jpg');order:2;" class="item-1-mosaico"></div><!--item1-mosaico-->

            <div class="item-2-mosaico" style="order:1;">
                <h2>Sun YISHAN</h2>
                <p><br/><br/><br/><br/><br/><br/><br/><br/></p>
            </div><!--item2-mosaico-->

        </div><!--flex-mosaico-->
    </div>
</section>